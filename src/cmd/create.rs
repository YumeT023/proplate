use std::{io::{Error, ErrorKind::AlreadyExists}, path::{PathBuf, Path}, fs};
use uuid::Uuid;

use crate::{template::{find::find_template_by_id, ForkTemplate}, colors::{title, step, success, error}, shell};
...
pub fn create(template_id: &str, dest: &str) -> Result<(), Error> {
  ...
  fs::create_dir_all(dest).map_err(|_| {
    cleanup();
    Error::new(AlreadyExists,  error(&format!("out dir already exists: {dest}")))
  })?;
  ...
  println!("{}", success("Done, wait a lil moment while we remove temporary files"));
  cleanup();
