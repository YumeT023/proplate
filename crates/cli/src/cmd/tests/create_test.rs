  ...
  fn unlicensed() {
    run_isolated_test(
      || {
        ...
        let (snap, _) = get_sample("conditional_ops", "unlicensed-snapshot");
  ...
  fn bsd_2_clause() {
    run_isolated_test(
      || {
        ...
        let (path, _) = test_create!("additional_ops", "conditional-license", ctx);
        // we got custom snap path here
        let (snap, _) = get_sample("conditional_ops", "bsd-2-clause-license-snapshot");
  ...
  fn mit() {
    run_isolated_test(
      || {
        ...
        let (path, _) = test_create!("additional_ops", "conditional-license", ctx);
        // we got custom snap path here
        let (snap, _) = get_sample("conditional_ops", "mit-license-snapshot");
